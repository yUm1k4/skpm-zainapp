...
class KategoriPengaduan extends BaseController
{
    ...
    public function simpandata()
    {
        if ($this->request->isAJAX()) {
            ...
            $valid = $this->validate([
                'nama_kategori'  => [
                    'label' => 'Nama Kategori',
                    'rules' => 'required|min_length[3]|max_length[40]|is_unique[pengaduan_kategori.nama_kategori]',
                    'errors' => [
            ...
            if (!$valid) {
                ...
                $simpandata = [
                    'nama_kategori' =>  ucfirst($this->request->getVar('nama_kategori'))
                ];
    ...
    public function updatedata()
    {
        if ($this->request->isAJAX()) {
            ...
            $row = $this->katModel->find($nama_kategori);

            // validasi utk field yg unique
            if ($row['nama_kategori'] == $this->request->getVar('nama_kategori')) {
                $rule_nama_kat = 'required|min_length[3]|max_length[40]';
            } else {
                $rule_nama_kat = 'required|min_length[3]|max_length[40]|is_unique[pengaduan_kategori.nama_kategori]';
            }
            // Validasi
            $validation = \Config\Services::validation();
            $valid = $this->validate([
